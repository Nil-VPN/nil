...
fn accepts_when_measurement_is_provably_logged() {
    ...
    let verdict = appraise(&ev, SPKI, &policy_logged(M, sk.verifying_key().to_bytes()), &NONCE)
...
fn rejects_when_log_pinned_but_no_proof_stapled() {
    ...
    let ev = synthetic_evidence(Tee::SevSnp, &M, SPKI, &NONCE);
    let err = appraise(&ev, SPKI, &policy_logged(M, log_key().verifying_key().to_bytes()), &NONCE)
        .unwrap_err();
    assert!(matches!(err, AttestError::TransparencyNotLogged(_)), "got {err:?}");
}
...
fn rejects_proof_for_a_different_measurement() {
    ...
    let ev = synthetic_evidence_logged(Tee::SevSnp, &M, SPKI, &NONCE, &proof);
    let err = appraise(&ev, SPKI, &policy_logged(M, sk.verifying_key().to_bytes()), &NONCE)
        .unwrap_err();
    assert!(matches!(err, AttestError::TransparencyNotLogged(_)), "got {err:?}");
}
