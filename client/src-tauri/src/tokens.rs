...
impl TokenClient {
    ...
    pub async fn acquire(&self, payment_id: &str) -> Result<StoredToken, TokenError> {
        ...
        let pk: PubKeyResponse = self
            ...
            .map_err(net_err)?;
        let pubkey_der =
            from_hex(&pk.public_der).ok_or_else(|| TokenError::Crypto("issuer pubkey not hex".into()))?;
        ...
        let req =
            nil_crypto::token::blind(&pubkey_der, &msg).map_err(|e| TokenError::Crypto(e.to_string()))?;

        // 3. Payment-gated blind-sign. 402 = not-yet-confirmed; 409 = already issued for this payment.
        let issue = IssueRequest { payment_id: payment_id.to_owned(), blind_msg: to_hex(&req.blind_msg) };
        let resp = self
            .http
        ...
        let blind_sig =
            from_hex(&issued.blind_sig).ok_or_else(|| TokenError::Crypto("blind_sig not hex".into()))?;
...
mod tests {
    ...
    fn stored_token_roundtrips_and_holds_only_msg_and_token() {
        let t = StoredToken { msg: "aa".into(), token: "bb".into() };
